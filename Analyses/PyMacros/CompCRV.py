...
def EventStr(event):
    ...
    eventStr = (
        ...
        f"crvcoincstimeStart: {event[f'crvcoincs.timeStart']}\n"
...
# To handle these two different array structures together we need to unfold klfit and klkl
...
def MarkTrackerCutsTest(data_): 

    data_["goodtrk"] = data_["kl.status"] >= 0

    data_["CRV1"] = (ak.num(data_["crvcoincs.sectorType"]) > 0) & (ak.any(data_["crvcoincs.sectorType"] == 1)) 
...
def flatten(array):
    ...
    while ak.type(array) == ak.types.ListType:
        array = ak.flatten(array)
...
def Run(finName):
    ...
    branches = ["crvcoincs.sectorType", "klfit"]
    ...
    # x = ak.ravel(tcrvpg_["klfit"]["pos"]["fCoordinates"]["fX"]) # , axis=2)
