...
def FilterParticles(data_, particle):
    ...
    # Should this not also be "all" instead of "any"?
...
def GetModule(data_, module):

    print(f"\n---> Getting module {module}") 

    # Enforce module condition
    moduleCondition = ak.any(data_[ut.coincsBranchName+".sectorType"] == module, axis=1)
    
    print(moduleCondition)
    # Mask
    data_ = data_[moduleCondition]
...
def WritePEsPerLayerToFile(data_, doutTag, foutTag, reproc):

    foutName = f"../Txt/{reproc}/PEsPerLayer/{doutTag}/PEsPerLayer_{foutTag}.csv"
...
def Run(finName, particle, reproc): 
    ...
    foutName = f"../Txt/{reproc}/PEsPerLayer/{doutTag}/PEsPerLayer_{foutTag}.h5"
...
def main():
    ...
    Run(finName=finName, particle=particle, reproc=reproc) 
